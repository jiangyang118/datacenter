...
class Worker
{
    ...
    protected static function writeStatisticsToStatusFile()
    {
        ...
        if (static::$_masterPid === posix_getpid()) {
            ...
            foreach(static::$_pidMap as $worker_id => $pid_array) {
                /** @var /Workerman/RowingMachine $worker */
                $worker = static::$_workers[$worker_id];
    ...
    protected static function writeConnectionsStatisticsToStatusFile()
    {
        ...
        if (static::$_masterPid === posix_getpid()) {
            ...
            file_put_contents(static::$_statisticsFile, "PID      RowingMachine          CID       Trans   Protocol        ipv4   ipv6   Recv-Q       Send-Q       Bytes-R      Bytes-W       Status         Local Address          Foreign Address\n", FILE_APPEND);
            chmod(static::$_statisticsFile, 0722);
            foreach (static::getAllWorkerPids() as $worker_pid) {
    ...
    public static function checkErrors()
    {
        if (static::STATUS_SHUTDOWN != static::$_status) {
            $error_msg = static::$_OS === OS_TYPE_LINUX ? 'RowingMachine['. posix_getpid() .'] process terminated' : 'RowingMachine process terminated';

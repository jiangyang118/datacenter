 ...
 * @method void exception($msg) static 记录异常日志
 */
class Log
{
    const LOG    = 'log';
     ...
     * @var array 日志类型
     */
    protected static $type = ['log', 'error', 'info', 'sql', 'notice', 'alert', 'debug', 'exception'];
     ...
     * @param  array $other_log_info 其他信息
     * @return bool
     */
    public static function write_do($msg, $type = 'log', $other_log_info = []) {
        if (!empty($other_log_info)) {
            $msg .= 'other_log_info:'.json_encode($other_log_info);
        }
        return self::write($msg, $type);
    }

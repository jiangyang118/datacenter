...
class Handle
{
    ...
    public function report(Exception $exception)
    {
        if (!$this->isIgnoreReport($exception)) {
            ...
            if (Config::get('log')['type'] != 'FormatFile') {
        ...
        if (Config::get('log')['type'] == 'FormatFile') {
            ...
            //错误等级
            $severity = 0;
            if ($exception instanceof ErrorException || $exception instanceof \ErrorException) {
                $severity = $exception->getSeverity();
            }
            //运行时致命的错误。不能修复的错误。停止执行脚本。E_ERROR。1
            //编译时的解析错误。解析错误应该只由解析器生成。E_PARSE。4
            //PHP 启动时的致命错误。E_CORE_ERROR。16
            //编译时致命的错误。E_COMPILE_ERROR。64

...
class SyncSqlDataGuizhou extends Command{
    protected function configure()
    {
        $this->setName('SyncSqlDataGuizhou')->setDescription('this api is for SyncSqlDataGuizhou')
    ...
    /**
     * sql_server数据同步脚本
     * php think SyncSqlDataGuizhou gzty 1 天津机能设备同步脚本-西门子尿十项分析仪Clinitek Status
     * php think SyncSqlDataGuizhou gzty 2 天津机能设备同步脚本-西门子全自动化学发光免疫分析仪ADVIA® Centaur CP
     * 瑞美sql_server添加两个字段
...
//        $business = $input->getArgument('business');
        ...
        switch ($op) {
            case 1:
                $this->sync_data($business, 'CPT');//希森美康
                break;
            case 2:
                $this->sync_data($business, 'DP8088');//东塘
                break;
            default:
                break;
        }
...
//        $db_mysql = get_db($mysql_database, $mysql_prefix,$mysql_hostname);
        ...
        foreach ($test_data as $item) {
            ...
            if (!empty($item['bz2'])) {
                ...
                $bz2_k8 = date('Y-m-d H:i:s', strtotime($item['bz2']));
                $date = date('Y-m-d', strtotime($bz2));
            } else {
                $bz2 = date('Y-m-d', strtotime($item['jyrq'])) . ' ' .  date('H:i:s');
                $bz2_k8 = date('Y-m-d', strtotime($item['jyrq'])) . ' ' .  date('H:i:s');
                $date = date('Y-m-d', strtotime($bz2));
            }
            //todo
            //判断是否可使用系统时间
            if($relation_type[$ruimei_yq] == 50){
                $bz2 = date('Y-m-d H:i:s', strtotime($item['operdt']));
                $date = date('Y-m-d', strtotime($item['operdt']));
            }
            unset($item['operdt']);
            $yq = trim($item['yq']);
            $ybh = trim($item['ybh']);
            ...
            } else if ($relation_type[$ruimei_yq] == 50) {//P8088设备
//                $xmdh = 'PDW(fl)';
                //设备本地时间可能错误，默认测试时间为同步时间
//                $bz2 = date('Y-m-d H:i:s', time());
            }
            $xmdh = !empty($xmdh_check[$xmdh]) ? $xmdh_check[$xmdh] : $xmdh;
            if (!isset($format_data[$date_ybh])) {
                $format_data[$date_ybh] = [
                    'staff_code' => '',//用户系统对应样本号
                    'yq' => $yq,
                    'ybh' => $ybh,
                    'bz2' => $bz2,
                    'date' => $date,
        ...
        /*
         * 统一字段
        [k1] => 111-------------------用户系统对应人员编号 ydy_specimen.specimen_no
        [k2] => CNTR_C----------------瑞美仪器代号 ydy_device_manage.ruimei_yq
        [k3] => 20231012104-----------仪器人员编号
        ...
        */
        foreach ($result_list as $item) {
            ...
            $item['relation_type'] = isset($relation_type[$ruimei_yq]) ? $relation_type[$ruimei_yq] : 0;
            $item['create_by'] = 'cron';
            //样本号
            $item['k1'] = '';
            $item['staff_uuid'] = '';
...
//            }
            //查询一下当前人员编号
            $current_staff = $db_mysql->name('staff')
                ->alias('a')
                ->join('staff_department b','a.uuid = b.staff_uuid','left')
                ->join('department c','b.department_uuid = c.uuid','left')
                ->where(['a.personnel_umber' => $item['k3']])
                ->field('a.uuid,c.uuid as department_uuid,a.personnel_umber')->find();
            if (!empty($current_staff)) {
                $item['k1'] = $current_staff['personnel_umber'];
                $item['staff_uuid'] = $current_staff['uuid'];
                $item['department_uuid'] = $current_staff['department_uuid'];
            }
